...
app.post("/app/new/interaction", (req, res) => {	
	...
	const stmt = db.prepare("INSERT INTO currentUser (user, pass, time) VALUES (?, ?, ?)")
	const info = stmt.run(data.user, data.pass, data.time);
	res.status(201).json({"message":info.changes +" record created: ID " + info.lastInsertRowid + " (201)"});
});

// READ a single user (HTTP method GET) at endpoint /app/user/:id
app.get("/app/interaction/:id", (req, res) => {	
	const stmt = db.prepare("SELECT * FROM currentUser WHERE id = ?").get(req.params.id);
...
app.delete("/app/delete/interaction/:id", (req, res) => {	
	const stmt = db.prepare("DELETE FROM currentUser WHERE id = ?").run(req.params.id);
